...
[![tools](https://img.shields.io/badge/tools-256-22C55E)](#-tools)
...
- 🛰️ **All four APIs** — Trading, Data, Broker, AuthX — up to **256 tools**.
...
| `ALPACA_TOOLSETS` | | `trading,data` | Comma-separated subset to expose: `trading,data,broker,authx`. Add `broker,authx` for the full ~256-tool surface. |
...
Every enabled Alpaca operation is registered as its own tool, `alpaca_<operationId>`. By default the server registers the `trading` and `data` toolsets (**101 tools**); set `ALPACA_TOOLSETS=trading,data,broker,authx` to expose the full **256-tool** surface across all four APIs. Each tool's input is a Zod schema generated from the OpenAPI spec, with arguments grouped into `pathParams`, `queryParams`, and `bodyParams`, and validated before the call:
